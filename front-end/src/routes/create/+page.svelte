<script lang="ts">
	...
	const dropTargetClassesMain = [
		'ring-2',
		'ring-[hsl(var(--primary)/0.6)]',
		'bg-[hsl(var(--primary-container)/0.38)]'
	];
	const dropTargetClassesRed = [
		'ring-2',
		'ring-[hsl(var(--team-red)/0.65)]',
		'bg-[hsl(var(--team-red-soft)/0.85)]'
	];
	const dropTargetClassesBlue = [
		'ring-2',
		'ring-[hsl(var(--team-blue)/0.65)]',
		'bg-[hsl(var(--team-blue-soft)/0.85)]'
	];
	...
	function handleDndConsiderCards(cid: number, e: CustomEvent<DndDetail>) {
	...
	function handleDndFinalizeCards(cid: number, e: CustomEvent<DndDetail>) {
		if (!e?.detail?.items) return;
		const col = columnItems.find((c) => c.id === cid);
		if (!col) return;
		col.items = e.detail.items;
	...
	<div class="grid items-start gap-4 xl:grid-cols-[1fr_420px_1fr]">
		{#each columnItems.filter((c) => c.id === COL_RED) as column (column.id)}
			<Card.Root class="create-card create-team create-team-red rounded-3xl">
				...
				<Card.Content>
					<div
						class="team-zone team-zone-red grid max-h-[440px] min-h-[220px] content-start justify-items-center gap-2 overflow-y-auto rounded-2xl p-2"
						use:dndzone={{
							items: column.items,
							type: 'player',
							flipDurationMs,
					...
					>
						{#each column.items as item (item.id)}
							<div
								animate:flip={{ duration: flipDurationMs }}
								class={`team-chip team-chip-red group relative h-12 w-full max-w-[210px] cursor-grab select-none rounded-xl px-3 text-[15px] font-semibold text-foreground transition hover:border-[hsl(var(--team-blue)/0.45)] hover:bg-[hsl(var(--team-blue-soft)/0.62)] active:cursor-grabbing ${isCurrentPlayer(item) ? 'whoami-chip' : ''}`}
							>
								<div class="flex h-full items-center justify-between gap-2">
									<div class="flex min-w-0 items-center gap-2">
										<span class="truncate">{item.name}</span>
				...
				<div class="grid grid-cols-[1fr_auto_1fr] items-end gap-4">
					<div class="space-y-2 text-center">
						...
						<Input
							...
							type="number"
							bind:value={redScore}
							class="h-12 text-center text-lg font-semibold"
							placeholder="0"
							max={10}
							min={0}
						/>
					</div>
					<div class="pb-3 text-sm font-semibold uppercase tracking-[0.16em] text-muted-foreground">
						VS
					</div>
					<div class="space-y-2 text-center">
